...
class C2FCriticNetwork(nn.Module):
    ...
    def forward_each_level(
        ...
        # Value
        value_x = torch.cat([value_h, prev_action, level_id], -1)  # [B, D]
        ...
        # Value
        adv_x = torch.cat([adv_h, prev_action, level_id], -1)  # [B, D]
        # Process through MLP for each action sequence step
    ...
    def forward(
        ...
        B, L = prev_actions.size(0), prev_actions.size(1)

        # Reshape previous actions
        prev_actions = prev_actions.view(-1, L, self._actor_dim)  # [B, L, D]
